...
Este `README.md` describe el contenido de este notebook de Google Colab, que implementa un modelo de Regresión Lineal simple.
...
1.  **Importación de Librerías**: Carga las bibliotecas necesarias como `pandas`, `numpy`, `matplotlib`, `seaborn`, `sklearn.model_selection`, `sklearn.linear_model` y `sklearn.metrics`.
2.  **Carga de Datos**: Creación de un pequeño dataset sintético con 'área', 'habitaciones' y 'precio' de viviendas.
3.  **Exploración de Datos (EDA)**: Análisis descriptivo (`df.info()`, `df.describe()`) y visualización de relaciones entre variables (`sns.pairplot`, `sns.heatmap`).
...
6.  **Modelo de Regresión**: Entrenamiento de un modelo de `LinearRegression` de `scikit-learn`.
...
Se busca predecir la `clase` (sistema operativo) de un usuario basándose en las siguientes características de comportamiento:

*   `duracion`: Tiempo que el usuario pasa en la plataforma.
*   `paginas`: Número de páginas visitadas.
*   `acciones`: Cantidad de interacciones realizadas.
*   `valor`: Una métrica de valor o importancia del usuario.

## Metodología

1.  **Carga de Datos**: Se carga el dataset `usuarios_win_mac_lin.csv` en un DataFrame de Pandas.
2.  **Preparación de Datos**: División del dataset en conjuntos de entrenamiento y prueba.
3.  **Entrenamiento del Modelo**: Se utiliza `LogisticRegression` de Scikit-learn para entrenar el modelo.
4.  **Predicción**: Se realizan predicciones sobre el conjunto de prueba.
5.  **Evaluación del Modelo**: Se evalúa el rendimiento del modelo utilizando métricas como `Accuracy`, `Matriz de Confusión` y `Classification Report`.
6.  **Visualización**: Se genera un mapa de calor para la matriz de confusión para una mejor interpretación de los resultados.

## Resultados Clave

El modelo de Regresión Logística alcanzó una `Accuracy` del **67.6%** en este dataset. La matriz de confusión reveló que el modelo tiene un buen desempeño en la clasificación de usuarios de Windows y Linux, pero presenta dificultades para identificar correctamente a los usuarios de Mac (clase 1), lo que sugiere un área de mejora.
